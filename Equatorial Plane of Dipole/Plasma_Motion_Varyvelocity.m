function Plasma_Motion_Varyvelocity
clc
...
rnum=250;
rstart=linspace(r_min,r_max,rnum);
if v_abs(i)==0.1
for k=1:rnum
options2=odeset('RelTol',1.0e-10,'AbsTol',1.0e-14);
[t4,z4]=ode45(@GuidingCenter,[Tmin,t(end)],[rstart(k),0,0],options2,v0(2));
z4_mag=sqrt(sum(z4.^2,2));
arcT=acos((dot(z4(1,:),z4(end,:),2))./(z4_mag(1)*z4_mag(end)));%Rad
distance_arcTrace=2*pi*rstart(k)*(arcT/(2*pi));
distance_arcActual=2*pi*rstart(k)*(arc/(2*pi));
distance_arc(k)=abs(distance_arcActual-distance_arcTrace);
end
end
if v_abs(i)==0.2
for k=1:rnum
...
if v_abs(i)==0.1
set(gca, 'YScale', 'log')
xlabel('(r_{0}-r_{min})/(r_{max}-r_{min})');
ylabel('Tracing Error');
grid on;
hold on
xlim([0,1])
semilogy((rstart-r_min)/(r_max-r_min),distance_arc,'b');
semilogy((rstart-r_min)/(r_max-r_min),distance_arc*2^3,'k');
hold on
end
if v_abs(i)==0.2
semilogy((rstart-r_min)/(r_max-r_min),distance_arc,'r');
hold on
end
...
%loglog(v_abs,distance_gr(1)*(v_abs/v_abs(1)).^4,'m');
%loglog(v_abs,distance_gc(1)*(v_abs/v_abs(1)).^4,'m');%Velocity Graph 
...
xlim([0.01,3.02])
%loglog(v_abs,MUexact_I);
%loglog(v_abs,MUexact_gc(1)*(v_abs/v_abs(1)).^4,'m');
%loglog(v_abs,MUexact_I(134)*(v_abs/v_abs(134)).^6,'y');
...
figure; %figure 30
xlabel('Velocity');
ylabel('(r_{\mu}r_{GP}r_{DP} - r_{min})/(r_{max} - r_{min})');
grid on;
hold on;
plot(v_abs,r_mag_mu,'b');
plot(v_abs,r_mag_gyroperiod,'r');
plot(v_abs,r_mag_driftperiod,'g');
xlim([0.01,3.48])
legend('r_{\mu}','r_{GP}','r_{DP}');
