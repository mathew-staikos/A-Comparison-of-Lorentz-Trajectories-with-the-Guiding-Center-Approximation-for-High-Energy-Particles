...
f3= @(lam) ((cos(lam)).^6./(sqrt(1+3.*(sin(lam)).^2)))-(sin(alpha(1)+(-0.0396*alpha(1)^2-0.1358*alpha(1)+ 0.3167))).^2;
...
Bounce_AnMod(i)=L*Re*1.0E3*(3.6-1.6*sin(alpha(1)+(-0.0396*alpha(1)^2-0.1358*alpha(1)+ 0.3167)))/(sqrt(W/m)); %s Modified Analytical 
...
Drift_AnMod(i)=(pi*q*M*(Re*1.0E3)^2)./(3*L*W*(0.35+0.15*sin(alpha(1)+(-0.0396*alpha(1)^2-0.1358*alpha(1)+ 0.3167)))); %s Modified Analytical 
...
%PLOTS
%Change in Pitch Angle as function of Pitch Angle 
figure; %FIGURE 75
hold on
xlim([0 pi/2])
xticks([0 pi/8 pi/4 3/8*pi pi/2])
xticklabels({'0','\pi/8','\pi/4','3\pi/8','\pi/2'})
plot(Pitch,AlphaDiff,'b');
xlabel('\alpha_{0} (Rad)');
ylabel('\Delta\alpha_{eq} (Rad)');
grid on;
%Average Line of Best Fit 
pf=polyfit(Pitch,AlphaDiff,2);
BF=polyval(pf,Pitch);
plot(Pitch,BF,'k');
%Minimum Line of Best Fit 
plot(Pitch,Pitch*0,'--k');
%Maximum Line of Best Fit  
peaks1=sign([0 diff(AlphaDiff)]);
idx1=strfind(peaks1,[1,-1]);
pf1=polyfit(Pitch(idx1),AlphaDiff(idx1),2);
BF1=polyval(pf1,Pitch);
plot(Pitch,BF1,'--k');

%Change in Magnetic Moment as function of Pitch Angle 
figure; %FIGURE 76
loglog(Pitch,muindex,'b');
hold on
xlim([0 pi/2])
xticks([0 pi/8 pi/4 3/8*pi pi/2])
xticklabels({'0','\pi/8','\pi/4','3\pi/8','\pi/2'})
...
Pitch(1)
muindex(1)
pf2=polyfit(log(Pitch(2:end)),log(muindex(2:end)),1)
BF2=polyval(pf2,log(Pitch));
plot(Pitch,exp(BF2),'k');

%GC approximation of Magnetic Mirror Latitude as function of Pitch Angle 
figure;  %FIGURE 78
plot(Pitch(5:end),GC_lam_Num(5:end),'b'); 
hold on
plot(Pitch,GC_lam_An,'r');
plot(Pitch,GC_lam_AnMod,'g');
xlim([0.012 pi/2])
xticks([0 pi/8 pi/4 3/8*pi pi/2])
xticklabels({'0','\pi/8','\pi/4','3\pi/8','\pi/2'})
xlabel('\alpha_{0} (Rad)');
...
%GC approximation of Bounce period as function of Pitch Angle 
figure; % FIGURE 79
semilogy(Pitch,Bounce_Num,'b');
hold on
semilogy(Pitch,Bounce_An,'r');
semilogy(Pitch,Bounce_AnMod,'g');
xlim([0 pi/2])
xticks([0 pi/8 pi/4 3/8*pi pi/2])
xticklabels({'0','\pi/8','\pi/4','3\pi/8','\pi/2'})
xlabel('\alpha_{0} (Rad)');
...
%GC approximation of Drift period as function of Pitch Angle 
figure; %FIGURE 80
semilogy(Pitch,Drift_Num,'b');
hold on
semilogy(Pitch,Drift_An,'r');
semilogy(Pitch,Drift_AnMod,'g');
xlim([0 pi/2])
xticks([0 pi/8 pi/4 3/8*pi pi/2])
xticklabels({'0','\pi/8','\pi/4','3\pi/8','\pi/2'})
xlabel('\alpha_{0} (Rad)');
