function VaryVelocity_HalfBounce
clc
...
%For line of best fit later
if v0_mag<9.01 %Re/s
v0_mag9(i)=v0_mag; %Rad
end
if v0_mag<9.01 %Re/s
AlphaDiff9(i)=(AlphaF-AlphaI); %Rad
end
...
f3= @(lam) ((cos(lam)).^6./(sqrt(1+3.*(sin(lam)).^2)))-(sin(alpha(1)+0.0366*v0_mag)).^2;
...
Bounce_AnMod(i)=L*Re*1.0E3*(3.6-1.6*sin(alpha(1)+0.0366*v0_mag))/(sqrt(W/m)); %s Modified Analytical 
...
Drift_AnMod(i)=(pi*q*M*(Re*1.0E3)^2)./(3*L*W*(0.35+0.15*sin(alpha(1)+0.0366*v0_mag))); %s Modified Analytical 
...
%PLOTS
%Change in Pitch Angle as function of velocity
figure; %FIGURE 65
hold on
plot(vv,AlphaDiff,'b');
...
%averge up to v = 9 Re/s
slope = v0_mag9(:)\AlphaDiff9(:);
plot(vv,vv*slope,'k');
%Minimum Line of Best Fit 
plot(vv,vv*0,'--k');
%Maximum Line of Best Fit  
peaks=sign([0 diff(AlphaDiff)]);
idx=strfind(peaks,[1,-1]);
v0_magM=vv(idx);
AlphaDiffM=AlphaDiff(idx);
slopeM=v0_magM(:)\AlphaDiffM(:);
plot(vv,vv*slopeM,'--k');

%Change in Magnetic Moment as function of velocity
figure; %FIGURE 66
plot(vv,muindex,'b');
hold on
xlabel('Velocity (Re/s)');
...
grid on;
%Average Line of Best Fit 
pf=polyfit(v0_mag9,muindex9,2);
BF=polyval(pf,vv);
plot(vv,BF,'k');
%Minimum Line of Best Fit 
plot(vv,vv*0,'--k');
%Maximum Line of Best Fit  
peaks1=sign([0 diff(muindex)]);
idx1=strfind(peaks1,[1,-1]);
pf1=polyfit(vv(idx1),muindex(idx1),2);
v0_magM=linspace(0,vv(max(idx1)));
BF2=polyval(pf1,v0_magM);
plot(v0_magM,BF2,'--k');

%GC approximation of Magnetic Mirror Latitude as function of velocity
figure;  % FIGURE 72
plot(vv,GC_lam_Num,'b');
hold on
plot(vv,GC_lam_An,'r');
plot(vv,GC_lam_AnMod,'g');
xlabel('Velocity (Re/s)');
ylabel('\lambda_{m} (Rad)');
legend('Numerical', 'GC Theory','Modified GC Theory')
grid on;

%GC approximation of Bounce period as function of velocity
figure; % FIGURE 73
semilogy(vv,Bounce_Num,'b');
hold on
semilogy(vv,Bounce_An,'r');
semilogy(vv,Bounce_AnMod,'g');
xlabel('Velocity (Re/s)');
ylabel('Bounce Period (s)');
grid on;
legend('Numerical', 'GC Theory','Modified GC Theory')

%GC approximation of Drift period as function of velocity
figure; %FIGURE 74
semilogy(vv,Drift_Num,'b');
hold on
semilogy(vv,Drift_An,'r');
semilogy(vv,Drift_AnMod,'g');
xlabel('Velocity (Re/s)');
